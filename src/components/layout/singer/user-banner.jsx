...
import { getUserSession, getAllEvents } from "@/lib/data";
                            ...
                            <div className="flex mt-20 ml-16 event-card rounded-lg w-96 h-full bg-customOrange text-white flex mx-4 my-2" style={{ transition: 'transform 0.3s ease' }}>
                                <div key={event.id}>
                                    ...
                                    <div className="flex justify-center pb-5">
                                        <span onClick={() => joinEvent(event)} className={`fa-regular ${selectedEvent ? 'fa-thumbs-up' : 'fa-thumbs-down'} cursor-pointer ${selectedEvent === event ? 'text-green-500  active:animate-bounce' : ''}`}>
                                            {selectedEvent === event ? '   Asistire' : '   No asistire'}
                                        </span>
                                    </div>
                                </div>
                            </div>
