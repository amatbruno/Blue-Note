...
export default function ShowAllEvents() {
  ...
  return (
    <div className="flex flex-col items-center over">
      ...
      <div className="flex flex-wrap justify-center w-3/5" style={{ alignItems: 'stretch' }}>
        {loading ? (
          <Spinner />
        ) : (
          events.map((event) => (
            <div
              key={event.event_id}
              className="event-card rounded-lg w-72 h-full bg-blue-500 text-white flex flex-col mx-4 my-2"
